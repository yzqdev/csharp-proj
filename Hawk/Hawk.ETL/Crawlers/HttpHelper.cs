...
namespace Hawk.ETL.Crawlers
{
    ...
    public class HttpHelper
    {
        ...
        public  async Task<HttpResponse> GetHtml(HttpItem requestitem, string url = null, string post = null)
        {
            HttpResponse response = new HttpResponse();
            try
            {
                var request = SetRequest(requestitem, url, post);
                response = await GetHttpRequestData(request, requestitem);
                ...
                if (!IsSuccess(code))
                {
                    ...
                    response.Html= GlobalHelper.Get("key_113") + code;
