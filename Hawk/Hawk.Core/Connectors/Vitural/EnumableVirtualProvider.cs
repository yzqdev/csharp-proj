...
{
    public class EnumableVirtualProvider<T> : IItemsProvider<T>,IEnumerable<T>
    {
        ...
        public EnumableVirtualProvider(IEnumerable<T> items, int count = -1)
        {
            if (count != -1)
                totalcount = count;
            enumerable = items;
