...
namespace KeqingNiuza.RealtimeNotes.Service {
    ...
    public static class DailyCheckTask {
        ...
        public static async System.Threading.Tasks.Task CheckIn() {
          ...
          Program.PrintLog = (log) => checkLog += $"[{DateTime.Now:HH:mm:ss}]{log}\n";
            if (!File.Exists($@"{Const.UserDataPath}\DailyCheckCookies")) {
                return;
            }
            try {
                var bytes = File.ReadAllBytes($@"{Const.UserDataPath}\DailyCheckCookies");
                ...
                File.AppendAllText($@"{Const.LogPath}\DailyCheck.txt", log);
            } catch (Exception ex) {
                ...
                Directory.CreateDirectory(Const.LogPath);
                File.AppendAllText($@"{Const.LogPath}\DailyCheck-Error.txt", errorLog);
            }
        }
    }
}
